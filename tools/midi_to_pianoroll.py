...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python3 midi_to_pianoroll.py <midi_file.mid> [bars]")
        print("\nExample: python3 midi_to_pianoroll.py song.mid 8")
        sys.exit(1)
    ...
    basename = Path(midi_file).stem
    tex_output = PROJECT_ROOT / "packets" / "pianorolls" / "_generated" / f"{basename}_pianoroll.tex"
