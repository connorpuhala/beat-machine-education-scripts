...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python3 multi_midi_to_sheet.py <midi_directory> [song_name]")
        print("\nExample: python3 multi_midi_to_sheet.py midi/come_together \"Come Together\"")
        sys.exit(1)
    
    midi_dir = sys.argv[1]
    song_name = sys.argv[2] if len(sys.argv) > 2 else os.path.basename(midi_dir).replace('_', ' ').title()
    
    if not os.path.isdir(midi_dir):
        print(f"Error: Directory '{midi_dir}' not found")
        sys.exit(1)
    
    # Analyze MIDI files
    analyzer = MultiMIDIAnalyzer(midi_dir)
    ...
    dir_name = os.path.basename(midi_dir.rstrip('/'))
    output_file = f"{dir_name}_pianoroll.tex"
